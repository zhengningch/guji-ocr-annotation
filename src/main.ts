...
async function refreshPresence(){if(!userToken)return;const el=document.querySelector<HTMLElement>('#onlineUsers');const {error}=await supabase.rpc('record_workspace_presence',{p_token:userToken});if(error){if(el){el.textContent='在线状态未启用';el.classList.add('offline')}return};const [{data:online},{data:members}]=await Promise.all([supabase.rpc('get_workspace_presence',{p_token:userToken}),supabase.rpc('get_task_participants',{p_token:userToken})]);if(Array.isArray(members))participants=members as typeof participants;if(el&&Array.isArray(online)){el.classList.remove('offline');el.textContent=`在线 ${online.length}：${online.map((x:{nickname:string})=>x.nickname).join('、')||'—'}`}}
...
function taskQuestions(task: Task): QuestionMap { const value=task.labels?.__questions;return value&&typeof value==='object'&&!Array.isArray(value)?value as QuestionMap:{} }
function taskMissing(task: Task) { return annotationFields.filter(name=>{const value=task.labels?.[name];return value==null||value===''||(Array.isArray(value)&&value.length===0)||value===CUSTOM_PREFIX||(Array.isArray(value)&&value.some(x=>x===CUSTOM_PREFIX))}) }
function taskStage(task: Task) { if(Object.keys(taskQuestions(task)).length)return '有疑问';if(!taskMissing(task).length)return '已完成';return task.status==='标注中'||task.corrected_text!==task.ocr_initial?'进行中':'未完成' }
function distribution(name:string){const counts:Record<string,number>={};tasks.forEach(task=>{const value=task.labels?.[name];(Array.isArray(value)?value:[value]).filter((x):x is string=>typeof x==='string'&&!!x&&x!==CUSTOM_PREFIX).forEach(x=>counts[x]=(counts[x]||0)+1)});return Object.entries(counts).sort((a,b)=>b[1]-a[1])}
